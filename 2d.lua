...
AdjointFlow_ElemDisc = NavierStokesAdjoint("q1,q2, h ", "outer")
...
PLaplaceDerivative_ApproxSpace:add_fct("u1,u2","Lagrange",1)
...
sigma = AdvancedGridFunction(PLaplaceDerivative_ApproxSpace);sigma:set(0.0);
...
PLaplaceDerivative_ElemDisc = PLaplaceDerivative("u1,u2", "outer")
PLaplaceDerivative_ElemDisc:set_lambda_vol(lambda_vol)
PLaplaceDerivative_ElemDisc:set_lambda_barycenter(lambda_x,lambda_y,0.0)
...
PLaplaceDerivativeRHS_ElemDisc = PLaplaceDerivativeRHS("u1,u2", "outer")
PLaplaceDerivativeRHS_ElemDisc:set_kinematic_viscosity(visc)

PLaplaceDerivativeRHS_ElemDisc:set_lambda_vol(lambda_vol)
PLaplaceDerivativeRHS_ElemDisc:set_lambda_barycenter(lambda_x,lambda_y,0)
...
print("FOR M=1, Bvol")
--Volume derviative
BVolume_ElemDisc = VolumeConstraintSecondDerivative("u1,u2", "outer") 
...
XBarycenter_ElemDisc = XBarycenterConstraintSecondDerivative("u1,u2", "outer") 
...
YBarycenter_ElemDisc = XBarycenterConstraintSecondDerivative("u1,u2", "outer") 
...
LargeProblemRHS_ElemDisc = LargeProblemRHS("u1,u2", "outer")
LargeProblemRHS_ElemDisc:set_kinematic_viscosity(visc)

LargeProblemRHS_ElemDisc:set_lambda_vol(lambda_vol)
LargeProblemRHS_ElemDisc:set_lambda_barycenter(lambda_x,lambda_y,0.0)
...
--TODO:JPRIME DISCRETIZATION
        Jprime_ElemDisc=JPrime("u1,u2","outer")
...
rhs = matrix(m,1)
DeltaLambda=matrix(3,1)
NegativeB = AdvancedGridFunction(PLaplaceDerivative_ApproxSpace);NegativeB:set(0.0);

ReferenceVolume = VolumeDefect(u_p,0, "outer", "u1,u2",4,false,1,false)--used as a constant
...
if bReadInitialGuess > 0 then 
	...
	SetDomainCoordinatesFromGF(grid_positions,"u1,u2")
...
p_solver_failure = false--iteration stop
...
vDrag[step] = 0.5 * visc * Drag(u_zeros, v, "v1,v2","u1,u2","outer",3)
...
while (step <= numSteps) do
	...
	--TODO:J' gets assembled here 
		...
		SetZeroAwayFromSubset(Sensitivity,"u1,u2","obstacle_surface")
	...
	while(true) do--step size control loop
		...
		while (true) do--start p loop
			...
			while(ns_i <= nsMaxIts) do--nonlinear solver of relaxed p-problem
				...
				--forces update of u_p in the assemby
				--at set(0.0) the gfs are PST_CONSISTENT
				Bvol:set(0.0);Bx:set(0.0);By:set(0.0);
				--after assemble_defect the gfs are PST_ADDITIVE
				Bvol_DomainDisc:assemble_defect(Bvol,u_p);Bx_DomainDisc:assemble_defect(Bx,u_p);By_DomainDisc:assemble_defect(By,u_p)	
				...
				--rhs[1][1]= -L_lambda[1][1] - (math.pow(step_control, p_current-1.0))*BTranspose_sigma[1][1]
				--rhs[2][1]= -L_lambda[2][1] - (math.pow(step_control, p_current-1.0))*BTranspose_sigma[2][1]
				--rhs[3][1]= -L_lambda[3][1] - (math.pow(step_control, p_current-1.0))*BTranspose_sigma[3][1]
				rhs:print()
				...
				--LargeProblem_DomainDisc:assemble_linear(A_Large, MinusLu_BdeltaLambda)
				...
				VecScaleAdd2(MinusLu_BdeltaLambda,1.0,MinusLu_BdeltaLambda,1.0,Sensitivity)
				--SetZeroAwayFromSubset(MinusLu_BdeltaLambda,"u1,u2","obstacle_surface")
				VecScaleAssign(MinusLu_BdeltaLambda, math.pow(step_control, p_current-1.0), MinusLu_BdeltaLambda)
				...
				if bDebugOutput then
					...
					vtkWriter:clear_selection()
					vtkWriter:select_nodal("u1,u2","up")
					vtkWriter:print("RHSBigProb_"..step.."_p_"..p_as_string, MinusLu_BdeltaLambda,ns_i,ns_i, false)
				end
				...
				delta_u_p_1=L2Norm(delta_u_p,"u1",4,"outer")
				delta_u_p_2=L2Norm(delta_u_p,"u2",4,"outer")
				delta_u_p_norm_sum=math.sqrt(delta_u_p_1*delta_u_p_1+delta_u_p_2*delta_u_p_2)
				...
				L_lambda[1][1]=VolumeDefect(u_p, ReferenceVolume, "outer", "u1,u2",4,false,1,false)
				vBarycenterDefect = BarycenterDefect(u_p,"u1,u2","outer",4)
				...
				PLaplaceDerivative_ElemDisc:set_lambda_barycenter(Lambda[2][1],Lambda[3][1],0)
				
				PLaplaceDerivativeRHS_ElemDisc:set_lambda_vol(Lambda[1][1])
				PLaplaceDerivativeRHS_ElemDisc:set_lambda_barycenter(Lambda[2][1],Lambda[3][1],0)
				
				LargeProblemRHS_ElemDisc:set_lambda_vol(Lambda[1][1])
				LargeProblemRHS_ElemDisc:set_lambda_barycenter(Lambda[2][1],Lambda[3][1],0)
		...
		if p_solver_failure == true then
			...
			--Update geometry
			TransformDomainByDisplacement(u_p, "u1,u2")
			...
			local drag_current = 0.5 * visc * Drag(u_zeros,v_temp,"v1,v2","u1,u2","outer",3)
			...
			Sensitivity:change_storage_type_to_consistent();
			Bvol:change_storage_type_to_consistent();Bx:change_storage_type_to_consistent();By:change_storage_type_to_consistent();
			...
			VecScaleAdd2(Sensitivity, 1.0, Sensitivity, 1.0, G_u);
			...
			if ((drag_current-drag_old) > 0.0
				...
				TransformDomainByDisplacement(u_p_negative, "u1,u2")
				steps_without_line_search = 0
				step_control = 0.5*step_control
				Lambda[1][1]=0.0
				Lambda[2][1]=0.0
